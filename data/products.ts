...
//data/products.ts
/*"use server";
import { fetchProducts, fetchCollection } from "@/lib/products";
...
export async function getProductsByIds(ids: string[], category?: string): Promise<Product[]> {
    const allProducts = await fetchProducts(category);
    return allProducts.filter((p) => ids.includes(p.id));
}

export async function getProductsUnderPrice(category: string, maxPrice: number): Promise<Product[]> {
    const products = await fetchProducts(category);
    return products.filter((p) => p.price <= maxPrice);
}

export async function getFeaturedProducts(featuredIds: string[]): Promise<Product[]> {
    return getProductsByIds(featuredIds);
}

export async function getProductsByCollection(title: string) {
    const collection = await fetchCollection(title);
    if (!collection) return [];
  
    const ids: string[] = collection.map((item) => item.id);
    const products = await fetchProducts(); // all categories
